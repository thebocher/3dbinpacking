...
class Item:
    ...
    def __init__(
            self, partno, name, typeof, WHD, weight, level, loadbear, updown,
            color, is_incoming
        ):
        self.partno = partno
        self.name = name
...
class Painter:
    ...
    def set_item_color(self, item, color):
        for artist in self.items_artists[item]:
            if not artist:
                continue 

            if isinstance(artist, art3d.PathPatch3D):
                artist.set_facecolor(color)

            artist.set_alpha(self.alpha)

    def update_incoming_items(self):
        for item in self.items_artists.keys():
            color = 'red' if item.is_incoming else 'gray'
            self.set_item_color(item, color)

    def _plotCube(
            self, ax, x, y, z, dx, dy, dz, color='red', mode=2, linewidth=1, 
            text="",fontsize=15,alpha=0.5, item=None):
        ...
        if mode == 1 :
            ...
            text_artist = None
            if text != "":
                text_artist = ax.text( (x+ dx/2), (y+ dy/2), (z+ dz/2), str(text),color='black', fontsize=fontsize, ha='center', va='center')

            self.items_artists[item] = [p, p2, p3, p4, p5, p6, text_artist]
    ...
    def plotItem(self, item, write_num, alpha, fontsize):
        x,y,z = item.position
        ...
        self._plotCube(
            self.axGlob, float(x), float(y), float(z), float(w), float(h), 
            float(d), color=color, mode=2, text=text,
            fontsize=fontsize,alpha=alpha, item=item
        )

    def removeItem(self, item):
        for artist in self.items_artists.get(item, []):
            if artist:
                artist.remove()

        del self.items_artists[item]

    def plotBoxAndItems(self,title="",alpha=0.2,write_num=False,fontsize=10, create_fig=True):
        self.alpha = alpha
        """ side effective. Plot the Bin and the items it contains. """
        if create_fig:
            fig = plt.figure()

        self.axGlob = axGlob = plt.axes(projection='3d')
        ...
        for item in self.items:
            ...
            if item.typeof == 'cube':
                 # plot item of cube
                self._plotCube(
                    axGlob, float(x), float(y), float(z), float(w), float(h), 
                    float(d), color=color, mode=2, text=text,
                    fontsize=fontsize,alpha=alpha, item=item
                )
